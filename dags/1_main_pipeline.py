...
from ingestion_covid import extract_covid_data  # We only import extract_covid_data
...
dag = DAG(
    ...
    description="Unified pipeline for COVID, Countries and Weather",
...
# Define tasks
save_last_success_time_task = PythonOperator(
    task_id="save_last_success_time_task",
    python_callable=save_last_success_time,
    dag=dag
)

# COVID
extract_covid_task = PythonOperator(task_id="extract_covid_task", python_callable=extract_covid_data, dag=dag)
# Removed validate_covid_task because validate_covid_data does not exist
transform_covid_task = PythonOperator(task_id="transform_covid_task", python_callable=transform_covid_data, dag=dag)
load_covid_task = PythonOperator(task_id="load_covid_task", python_callable=load_covid_data, dag=dag)
report_covid_task = PythonOperator(task_id="report_covid_task", python_callable=report_covid, dag=dag)
...
# Dependencies
extract_covid_task >> transform_covid_task >> load_covid_task >> report_covid_task >> save_last_success_time_task >> final_task
extract_countries_task >> transform_countries_task >> enrich_countries_task >> load_countries_task >> summarize_countries_task >> report_countries_task >> save_last_success_time_task
extract_weather_task >> transform_weather_task >> load_weather_task >> summarize_weather_task >> report_weather_task >> save_last_success_time_task
